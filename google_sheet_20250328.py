...
def get_response(user_id_or_group_id,user_message):  #回傳對應的回應內容 
    ...
    if user_message == "#紀錄表":
        return get_record_data(user_id_or_group_id)
...
def get_record_data(user_id_or_group_id):  #讀取紀錄表中項目欄
    spreadsheet, sheet_list = get_user_spreadsheet(user_id_or_group_id)
    if "紀錄表" in sheet_list:
        sheet = spreadsheet.worksheet("紀錄表")
        data = sheet.get_all_records()
        response_message = "📋【紀錄表】\n"
        if data:
            for record in data:
                # 取出項目名稱和結束時間
                item_name = record['項目名稱']
                end_time = record['結束時間']
                response_message += f"{item_name}_{end_time}\n"
